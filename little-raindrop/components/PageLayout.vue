<template>
  <div>
    ...
    <div id="anchorBar" class="anchor-catalog" :class="isFixedAnchorNav ? 'fixed-anchor-bar' : ''">
      <div class="container">
        <a
          ...
          class="anchor-title"
          :href="'#floor-' + (index + 1)"
          :class="{ 'anchor-active' : curTab === index }"
        >
    ...
    <div v-if="isFixedAnchorNav" :style="{'height': anchorNavHeight + 'px'}"></div>
...
export default {
  ...
  methods: {
    initSectionOffsetTop() {
      console.log('this.anchorNavHeight', this.anchorNavHeight)
    ...
    handleScrollAnchor() {
      ...
      // const isPageBottom = pageScrollTop + windowHeight === scrollHeight
      ...
      for (let i = 0; i < this.sectionOffsetTop.length; i++) {
        if (pageScrollTop >= this.sectionOffsetTop[i]) {
          activeIndex = i
        }
      }
      this.curTab = activeIndex
    },
